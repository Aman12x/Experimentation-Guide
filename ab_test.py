...
def simulate_users(n_users=1000, conv_rate_a=0.1, conv_rate_b=0.12, seed=42):
    ...
    users["converted"] = users.apply(
        lambda row: np.random.rand() < conversion_rates[row["group"]], axis=1
    )
    return users
...
def run_statistical_test(users, alternative="two-sided"):
    ...
    if p_norm_a < 0.05 or p_norm_b < 0.05:
        ...
        stat, p_val = proportions_ztest(
            count=success, nobs=nobs, alternative=alternative
        )
...
def run_bayesian_ab_test(
    df, group_col="group", metric_col="converted", group_a="A", group_b="B"
):
